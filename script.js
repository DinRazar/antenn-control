...
async function fetchTelemetry() {
    try {
        ...
        // Сохранение целевых значений для визуализации
        targetAz = tarAz;
        targetEl = tarEl;
        
        // Определение, нужно ли показывать диапазоны поиска (автоматическое наведение)
        ...
        // Перерисовка с учетом флага showSearchRange
        drawAzimuth(curAz);
...
function drawElevation(angle) {
    ...
    if (showSearchRange && searchEl > 0 && targetEl >= 0) {
        ...
        if (low < high) {
            // Преобразование углов в радианы для дуги
